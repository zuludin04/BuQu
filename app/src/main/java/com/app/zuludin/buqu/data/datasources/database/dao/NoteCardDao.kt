...
interface NoteCardDao {
    ...
    @Query("SELECT DISTINCT note_card.*, CASE WHEN rope.ropeId IS NOT NULL THEN 1 ELSE 0 END AS isConnected FROM note_card LEFT JOIN rope ON note_card.noteId = rope.sourceNoteId OR note_card.noteId = rope.targetNoteId WHERE note_card.boardId = :boardId")
    suspend fun getNotesByBoard(boardId: String): List<ConnectedNoteCardEntity>

    @Query("DELETE FROM note_card WHERE boardId = :boardId")
